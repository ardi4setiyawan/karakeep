...
import {
  ...
  integer,
  primaryKey,
  pgTable, // <-- Sudah pgTable
  text,
  unique,
} from "drizzle-orm/pg-core"; // <-- Sudah pg-core
...
export const bookmarks = pgTable(
  "bookmarks",
  {
    ...
    title: text("title"),
    archived: integer("archived", { mode: "boolean" }).notNull().default(0), // FIX: false -> 0
    favourited: integer("favourited", { mode: "boolean" })
      .notNull()
      .default(0), // FIX: false -> 0
    userId: text("userId")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
...
export const bookmarkLists = pgTable(
  "bookmarkLists",
  {
    ...
    public: integer("public", { mode: "boolean" }).notNull().default(0), // FIX: false -> 0
  },
  (bl) => [
...
export const rssFeedsTable = pgTable(
  "rssFeeds",
  {
    ...
    name: text("name").notNull(),
    url: text("url").notNull(),
    enabled: integer("enabled", { mode: "boolean" }).notNull().default(1), // FIX: true -> 1
...
// HANYA ADA SATU INSTANCE INI (FIX DUPLIKASI)
